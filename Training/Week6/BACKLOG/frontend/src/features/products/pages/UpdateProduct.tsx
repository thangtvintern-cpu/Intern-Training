...
const updateProductSchema = z.object({
    ...
    discountPercentage: z.number().min(1, "Discount percentage must be at least 1"),
})

export type UpdateProductType = z.infer<typeof updateProductSchema>


const UpdateProduct = ({ product, categories, onToggle }: UpdateProductProps) => {
    const { register, handleSubmit, formState: { errors, isSubmitting }, setError, reset } = useForm<UpdateProductType>({
    ...
    const onSubmit = async (data: UpdateProductType) => {
        try {
            ...
            setError("title", { type: "manual", message: "Failed to update product" })
        }
    }

    return (
        <div className="size-full flex flex-col p-4">
            <h1 className="text-2xl font-bold text-text-primary border-b border-border pb-2 mb-4">Edit Product</h1>
            ...
            <button className="custom-btn mt-auto" type="submit" disabled={isSubmitting} onClick={handleSubmit(onSubmit)}>
                {isSubmitting ? <Spinner className="size-6" /> : "Update"}
            </button>
